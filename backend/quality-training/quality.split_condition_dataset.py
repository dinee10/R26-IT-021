...
def split_condition_dataset(
    ...
    for species_dir in sorted(path for path in source_dir.iterdir() if path.is_dir()):
        for condition_dir in sorted(path for path in species_dir.iterdir() if path.is_dir()):
            images = [
            ...
            for split_name, split_images in splits.items():
                ...
                print(
                    f"{split_name}/{species_dir.name}/{condition_dir.name}: "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Split plant condition images into train, val, and test folders."
    )
