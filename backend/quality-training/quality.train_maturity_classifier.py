...
def load_evaluation_module():
    evaluate_script_path = Path(__file__).with_name(
        "quality.evaluate_maturity_classifier.py"
    )
    evaluate_spec = importlib.util.spec_from_file_location(
        "quality_evaluate_maturity_classifier",
...
def list_maturity_heads(train_dir: Path) -> dict[str, list[str]]:
    return {
...
def collect_files(split_dir: Path, heads: dict[str, list[str]]) -> list[tuple[str, str, int]]:
    ...
    for species, maturity_stages in heads.items():
        for stage_index, stage_name in enumerate(maturity_stages):
            stage_dir = split_dir / species / stage_name

            if not stage_dir.exists():
                continue

            for pattern in IMAGE_EXTENSIONS:
                for image_path in sorted(stage_dir.glob(pattern)):
                    examples.append((str(image_path), species, stage_index))
...
def build_model(model_name: str, heads: dict[str, list[str]]):
    ...
    shared = tf.keras.layers.Dropout(0.3)(base.output)
    outputs = {
...
def save_maturity_labels(output_dir: Path, model_name: str, heads: dict[str, list[str]]) -> None:
    labels_path = output_dir / f"quality.maturity_classifier.{model_name}.labels.json"
...
def print_dataset_counts(heads: dict[str, list[str]], train_dir: Path) -> None:
    for species, maturity_stages in heads.items():
        counts = Counter()
        for stage_name in maturity_stages:
            stage_dir = train_dir / species / stage_name
            counts[stage_name] = sum(
                len(list(stage_dir.glob(pattern)))
                for pattern in IMAGE_EXTENSIONS
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a plant-aware multi-head maturity classifier."
    )
    ...
    heads = list_maturity_heads(train_dir)
    ...
    model_path = args.output_dir / f"quality.maturity_classifier.{args.model}.keras"
    history_path = args.output_dir / f"quality.maturity_classifier.{args.model}.history.json"

    model.save(model_path)
    save_maturity_labels(args.output_dir, args.model, heads)
